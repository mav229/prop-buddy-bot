    ...
    const stream = new ReadableStream({
      async pull(controller) {
        ...
        if (done) {
          ...
          // --- Global cost alert: track TOTAL cost across all sessions, email every $0.10 ---
          try {
            ...
            // Use a global tracker (session_id="__global__", email="__cost__")
            ...
            await supabase
              .from("session_cache")
              .upsert(
                { session_id: "__global__", email: "__cost__", context_json: { total: newCost, last_alerted_at: newMilestone > prevMilestone ? newCost : lastAlertedAt } },
                { onConflict: "session_id,email" }
              );

            if (newMilestone > prevMilestone) {
              console.log(`[COST ALERT] Global cost hit $${newCost.toFixed(4)} (milestone ${newMilestone}) — sending email`);
              ...
              if (RESEND_API_KEY) {
                await fetch("https://api.resend.com/emails", {
                  ...
                  body: JSON.stringify({
                    ...
                    subject: `⚠️ AI Credit Alert: Total usage hit $${(newMilestone * 0.10).toFixed(2)}`,
                    html: `<h2>AI Credit Usage Alert</h2>
                      <p><strong>Total Spent:</strong> $${newCost.toFixed(4)}</p>
                      <p><strong>Milestone:</strong> $${(newMilestone * 0.10).toFixed(2)}</p>
                      <p><strong>Latest Session:</strong> ${sessionId.slice(0, 8)}...</p>
                      <p><strong>Source:</strong> ${channelSource}</p>
                      <p><strong>This Message:</strong> ~${estimatedInputTokens} input, ~${outputTokens} output ($${msgCost.toFixed(4)})</p>
